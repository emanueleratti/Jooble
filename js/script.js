...
function jobResearch(workPosition, geoPosition){
    // Definisco, un array vuoto, un counter e un oggetto vuoto che popolerò con i risultati di ricerca ottenuti
    let searchResult = [];
    let searchCounter = 0;
    ...
    for (let i = 0; i < jobs.length; i++){
        if (jobs[i].title.toLowerCase().includes(workPosition) && jobs[i].location.toLowerCase().includes(geoPosition)){
            searchResult.push({ title: jobs[i].title, location: jobs[i].location });
            searchCounter++;
        }
    }
